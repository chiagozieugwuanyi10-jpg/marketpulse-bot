...
def get_ai_usage_today(chat_id):
    ...
    try:
        ...
        c.execute(
            "SELECT COUNT(*) FROM feature_usage WHERE chat=? AND feature='ai_question' AND timestamp LIKE %s",
            (str(chat_id), today + "%")
        )
        count = c.fetchone()[0]
        db.close()
...
def ban_user(chat_id, reason="No reason provided"):
    ...
    try:
        ...
        c = db.cursor()
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        c.execute("INSERT OR REPLACE INTO banned_users (chat, reason, banned_at) VALUES (%s,%s,%s)",
    ...
    try:
        ...
        for coin in COINS:
            ...
            if price and price > 0:
                c.execute("INSERT INTO history (coin, price, timestamp) VALUES (%s, %s, %s)",
                          (coin, price, now))
...
TONE: Confident and direct. Sound like a sharp analyst talking to a serious trader. No padding, no generic phrases.

STRICT FORMATTING RULES:
- NEVER use asterisks (*) for anything. Not for bold, not for bullets, not for emphasis.
- Use Telegram HTML bold tags <b>key term</b> to highlight important numbers, coin names, and key levels.
- Plain text for everything else. No markdown.

FORMAT (follow exactly):
SITUATION: One sentence — what is happening. Bold the key price or level like <b>$60,000</b>.
CONTEXT: One sentence — why it matters for Nigerian traders. Bold the key naira figure if relevant.
DECISION: Entry: <b>$X</b>  Stop: <b>$Y</b>  Target: <b>$Z</b>. Or: Wait — [one clear reason].

End with: NFA — manage your risk.
Max 6 lines. Be specific and honest.
"""
...
def check_user_price_alerts():
    """Check all active user-set price alerts (above/below target) for any coin."""
    try:
        db = get_db()
        c = db.cursor()
        ...
        for row in rows:
            ...
            try:
                ...
                if triggered:
                    ...
                    # Deactivate so it doesn't keep firing
                    db2 = get_db()
                    c2 = db2.cursor()
                    c2.execute("UPDATE alerts SET active=0 WHERE id=%s", (aid,))
                    db2.commit()
                    db2.close()
...
def check_watchlist_alerts():
    try:
        ...
        for (chat_id,) in users:
            c.execute("SELECT coin FROM watchlists WHERE chat=%s", (chat_id,))
            coins = c.fetchall()
            for (coin,) in coins:
                price, change = get_best_price(coin)
...
KEY_ALERT_COINS = ["BTC", "ETH", "SOL", "BNB", "XRP", "DOGE"]

KEY_LEVELS = {
    # BTC — major psychological and historical levels
    "BTC":  [110000,105000,100000,95000,90000,85000,80000,75000,70000,
             68000,65000,63000,60000,58000,55000,50000,45000,40000,30000,20000],
    # ETH — key levels around current ~$3,400 range
    "ETH":  [5000,4500,4000,3800,3500,3400,3200,3000,2800,2500,2000,1800,1500,1000],
    # SOL — current range ~$150-160
    "SOL":  [300,250,200,180,170,160,150,140,130,120,100,80,50],
    # BNB — current range ~$580-620
    "BNB":  [1000,800,700,650,620,600,580,560,500,400,300,200],
    # XRP — current range ~$0.50-0.55
    "XRP":  [5.0,4.0,3.0,2.0,1.5,1.0,0.75,0.60,0.55,0.50,0.45,0.40,0.30],
    # DOGE — extra coin
    "DOGE": [1.0,0.50,0.30,0.20,0.15,0.10,0.08],
}

def _nearest_key_level(price, levels, tolerance=0.018):  # 1.8% tolerance
    for level in levels:
...
def daily_digest():
    try:
        ...
        c = db.cursor()
        today = datetime.now().strftime("%Y-%m-%d")
        c.execute("SELECT COUNT(*) FROM events WHERE timestamp LIKE %s", (today + '%',))
        total_events = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM users WHERE first_seen LIKE %s", (today + '%',))
        new_users = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM users WHERE last_seen LIKE %s", (today + '%',))
        ...
        try:
            ...
            if CHANNEL_ENABLED:
                if wat_h == SCHEDULE["morning_hour_wat"] and not morning_posted:
                    ...
                    if get_bot_mode() == "everyone":
                        post_to_channel(build_morning_briefing_pro())
                    else:
                ...
                if wat_h == SCHEDULE["midday_hour_wat"] and not midday_posted:
                    ...
                    if get_bot_mode() == "everyone":
                        post_to_channel(build_midday_snapshot_pro())
                    else:
                ...
                if wat_h == SCHEDULE["evening_hour_wat"] and not evening_posted:
                    ...
                    if get_bot_mode() == "everyone":
                        post_to_channel(build_evening_recap_pro())
                    else:
                ...
                if (wat.weekday() == SCHEDULE["weekly_edge_day"] and
                    ...
                    if get_bot_mode() == "everyone":
                        post_to_channel(build_weekly_edge_pro())
                    else:
                        ...
                        if len(parts) >= 5:
                            try:
                                ...
                                if direction == "LONG":
                                    ...
                                    pnl = (entry_price - exit_price) * size
                                c.execute("UPDATE trade_journal SET pnl=%s WHERE id=lastrowid", (pnl,))
                                db.commit()
                                db.close()
                ...
                if "callback_query" in u:
                    ...
                    if data.startswith("lang_"):
                        ...
                        try:
                            ...
                            c = db.cursor()
                            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            c.execute("INSERT OR REPLACE INTO user_preferences (chat, language, updated_at) VALUES (%s,%s,%s)",
                    ...
                    if data == "settings_notifications":
                        try:
                            ...
                            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            c.execute("INSERT OR REPLACE INTO user_preferences (chat, notifications, updated_at) VALUES (%s,%s,%s)",
                    ...
                    if data == "settings_theme":
                        try:
                            ...
                            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            c.execute("INSERT OR REPLACE INTO user_preferences (chat, theme, updated_at) VALUES (%s,%s,%s)",
                                      (str(chat_id), new_val, now))
                            db.commit()
                            db.close()
                    ...
                    if data == "ap_morning" and chat_id in ADMIN_IDS:
                        ...
                        post_to_channel(build_morning_briefing_pro() if get_bot_mode()=="everyone" else build_morning_briefing())
                        post_to_pro_channel(build_morning_briefing_pro())
                    ...
                    if data == "ap_midday" and chat_id in ADMIN_IDS:
                        edit(chat_id, message_id, "⏳ Publishing...", None)
                        post_to_channel(build_midday_snapshot_pro() if get_bot_mode()=="everyone" else build_midday_snapshot())
                        post_to_pro_channel(build_midday_snapshot_pro())
                    ...
                    if data == "ap_evening" and chat_id in ADMIN_IDS:
                        edit(chat_id, message_id, "⏳ Publishing...", None)
                        post_to_channel(build_evening_recap_pro() if get_bot_mode()=="everyone" else build_evening_recap())
                        post_to_pro_channel(build_evening_recap_pro())
                    ...
                    if data == "ap_weekly" and chat_id in ADMIN_IDS:
                        edit(chat_id, message_id, "⏳ Publishing...", None)
                        post_to_channel(build_weekly_edge_pro() if get_bot_mode()=="everyone" else build_weekly_edge())
                        post_to_pro_channel(build_weekly_edge_pro())
