...
PRO_CHANNEL_ID = "-100XXXXXXXXX"  # Your Pro-only channel
...
def _morning_base(d):
    ...
    if d["buy"] and d["sell"]:
        lines += [f"💱 <b>USDT/NGN</b>  Buy \u20a6{int(d['buy'])}  |  Sell \u20a6{int(d['sell'])}", ""]
    return lines

def build_morning_briefing():
    """Free channel — prices and levels only, no AI."""
    d = _morning_data()
    lines = _morning_base(d)
    if d["btc_price"]:
        r = round(d["btc_price"] * 1.02, 2)
        s = round(d["btc_price"] * 0.98, 2)
        lines += ["━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "",
                  "🎯 <b>KEY LEVELS (BTC)</b>",
...
def build_morning_briefing_pro():
    """Pro channel — same base + AI analysis + trade idea."""
    d = _morning_data()
    lines = _morning_base(d)
    if d["btc_price"]:
        r = round(d["btc_price"] * 1.02, 2)
    ...
    fg_val = d["fg_data"][0]["value"] if d["fg_data"] else "N/A"
    ai_prompt = (
        f"Morning: BTC {format_price(d['btc_price'])} ({format_change(d['btc_change'])}), "
        f"ETH {format_price(d['eth_price'])} ({format_change(d['eth_change'])}), "
        f"SOL {format_price(d['sol_price'])} ({format_change(d['sol_change'])}). "
        f"Fear & Greed: {fg_val}/100. "
        f"Give a structured morning analysis: what is happening, why it matters, and what you would personally do as a trader today. "
        f"Include specific entry ideas, stop loss, and targets. Label your trade idea EDUCATIONAL ONLY."
    )
    ai, _ = ask_ai(ai_prompt)
    if not ai:
        ai = "Markets are active today. Watch key levels and manage risk carefully."
    lines += ["━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "",
              "🧠 <b>AI MORNING ANALYSIS</b>", "", ai, "",
              "\u26a0\ufe0f <i>🎓 EDUCATIONAL ONLY — What the AI would do as a trader. Not financial advice.</i>",
              "", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "",
              "<i>NFA - DYOR</i>",
              "⚡ Market Pulse Pro — @MarketNgPulseBot"]
    return "\n".join(lines)

def build_midday_snapshot():
    """Free channel — prices + top movers, no AI."""
    btc_price, btc_change = get_best_price("BTC")
    eth_price, eth_change = get_best_price("ETH")
    sol_price, sol_change = get_best_price("SOL")
    bnb_price, bnb_change = get_best_price("BNB")
    ...
    if gainers:
        ...
        for coin, price, chg in gainers[:3]:
            lines.append(f"  <b>{coin}</b>  {format_price(price)}  +{chg:.2f}%")
        lines.append("")
    if losers:
        ...
        lines.append("")
    if buy and sell:
        lines += [f"💱 <b>USDT/NGN</b>  Buy \u20a6{int(buy)}  |  Sell \u20a6{int(sell)}", ""]
    lines += ["━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "",
              "<i>NFA - DYOR</i>", "⚡ Market Pulse — @MarketNgPulseBot"]
    return "\n".join(lines)

def build_midday_snapshot_pro():
    """Pro channel — same + AI midday outlook."""
    btc_price, btc_change = get_best_price("BTC")
    eth_price, eth_change = get_best_price("ETH")
    sol_price, sol_change = get_best_price("SOL")
    bnb_price, bnb_change = get_best_price("BNB")
    ...
    if buy and sell:
        lines += [f"💱 <b>USDT/NGN</b>  Buy \u20a6{int(buy)}  |  Sell \u20a6{int(sell)}", ""]
    ai_prompt = (
        f"Midday check: BTC {format_price(btc_price)} ({format_change(btc_change)}), "
        f"ETH {format_price(eth_price)} ({format_change(eth_change)}), "
        f"SOL {format_price(sol_price)}. Fear & Greed: {fg_val}/100. "
        f"Give a midday analysis: what the afternoon could look like, key levels to watch, and what you would personally do as a trader right now. "
        f"Label trade idea EDUCATIONAL ONLY."
    )
    ai, _ = ask_ai(ai_prompt)
    if not ai:
        ai = "Markets are in midday consolidation. Watch key levels for afternoon direction."
    lines += ["━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "",
              "🤖 <b>AI MIDDAY OUTLOOK</b>", "", ai, "",
              "\u26a0\ufe0f <i>🎓 EDUCATIONAL ONLY — NFA - DYOR</i>",
              "", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "",
              "<i>NFA - DYOR</i>", "⚡ Market Pulse Pro — @MarketNgPulseBot"]
    return "\n".join(lines)

def build_evening_recap():
    """Free channel — prices + winners/losers, no AI."""
    btc_price, btc_change = get_best_price("BTC")
    eth_price, eth_change = get_best_price("ETH")
    fg_data = get_fear_greed()
    gainers, losers = get_gainers_losers()
...
def build_evening_recap_pro():
    """Pro channel — same + AI tomorrow outlook + trade idea."""
    btc_price, btc_change = get_best_price("BTC")
    eth_price, eth_change = get_best_price("ETH")
    fg_data = get_fear_greed()
    gainers, losers = get_gainers_losers()
    ...
    ai_prompt = (
        f"Evening wrap: BTC {format_price(btc_price)} ({format_change(btc_change)}), "
        f"ETH {format_price(eth_price)} ({format_change(eth_change)}). "
        f"Fear & Greed: {fg_val}/100. Top gainers: {g_str}. Top losers: {l_str}. "
        f"Give an evening analysis: what happened today, what tomorrow looks like, "
        f"and what you would personally do as a trader overnight/into tomorrow. "
        f"Include any setups to watch. Label EDUCATIONAL ONLY."
    )
    ai, _ = ask_ai(ai_prompt)
    if not ai:
        ai = "Markets held key levels today. Watch for continuation tomorrow."
    lines += ["━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "",
              "🔮 <b>TOMORROW'S OUTLOOK + AI ANALYSIS</b>", "", ai, "",
              "\u26a0\ufe0f <i>🎓 EDUCATIONAL ONLY — NFA - DYOR</i>",
              "", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "",
              "<i>NFA - DYOR</i>", "⚡ Market Pulse Pro — @MarketNgPulseBot"]
    return "\n".join(lines)
...
def build_whale_watch(coin, move):
    """Free channel — plain whale alert."""
    price, _ = get_best_price(coin)
...
def build_whale_watch_pro(coin, move):
    """Pro channel — whale alert + AI analysis."""
    price, _ = get_best_price(coin)
    direction = "🚀 PUMPING" if move > 0 else "🔴 DUMPING"
    sign = "+" if move > 0 else ""
    sd = get_secondary_coin(coin)
    ...
    fg_val  = fg_data[0]["value"] if fg_data else "N/A"
    ai_prompt = (
        f"{coin} is moving {sign}{move:.2f}% in the last hour, currently at {format_price(price)}. "
        ...
        f"Fear & Greed: {fg_val}/100. "
        f"This looks like a major move. Analyze: is this a breakout or a trap? "
        f"What would you personally do as a trader right now — buy, sell, wait, or hedge? "
        f"Give entry, stop loss, targets. Label EDUCATIONAL ONLY."
    )
    ai, _ = ask_ai(ai_prompt)
    if not ai:
        ai = "Major move detected. Wait for confirmation before entering."
    lines = [
        f"🐋 <b>WHALE WATCH PRO — {coin}</b>", "",
        f"<b>{coin}</b> is <b>{direction}</b>",
        f"  Move: <b>{sign}{move:.2f}%</b> in 1 hour",
        f"  Price: <b>{format_price(price)}</b>", "",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "",
        "🧠 <b>AI BREAKOUT ANALYSIS</b>", "", ai, "",
        "⚠️ <i>🎓 EDUCATIONAL ONLY — What the AI would do as a trader. Not financial advice.</i>",
        "", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "",
        "<i>NFA - DYOR</i>", "⚡ Market Pulse Pro — @MarketNgPulseBot"
    ]
...
def _nearest_key_level(price, levels, tolerance=0.015):
    for level in levels:
...
def build_free_key_alert(coin, price, change, level):
    ...
    lines = [
        ...
        "",
        f"{arrow} <b>{coin}</b> is testing a key {label} level",
...
def build_pro_key_alert(coin, price, change, level, ai_analysis):
    ...
    label = "RESISTANCE" if change >= 0 else "SUPPORT"
    lines = [
        f"🔔 <b>PRO ALERT — {coin} TESTING KEY LEVEL</b>",
        "",
        f"{arrow} <b>{coin}</b> is testing {label} at <b>{format_price(level)}</b>",
        f"💰 Current: <b>{format_price(price)}</b>  {format_change(change)}",
        "",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        "",
        "🧠 <b>AI ANALYSIS</b>",
        "",
        ai_analysis or "Analysis unavailable at this time.",
        "",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        "",
        "⚠️ <i>🎓 EDUCATIONAL ONLY — This is what the AI would do as a trader. Not financial advice.</i>",
        "",
        "<i>NFA - DYOR</i>",
        "⚡ Market Pulse Pro — @MarketNgPulseBot",
    ]
...
def check_key_market_alerts():
    ...
    try:
        for coin in KEY_ALERT_COINS:
            ...
            if state.get("level") == level and (now - state.get("time", 0)) < 14400:
                continue
            ...
            ai_prompt = (
                ...
                f"24h High: {h_str}  24h Low: {l_str}. "
                f"Fear & Greed: {fg_val}/100 ({fg_label}). "
                f"As a professional crypto trader, give a structured analysis: "
                f"1) What this level means, 2) What you would personally do right now as a trader "
                f"(entry point, stop loss, take profit targets), 3) What to watch for confirmation or rejection. "
                f"Label your personal trade idea as EDUCATIONAL ONLY."
            )
            ...
            for u in updates.get("result", []):
                ...
                if "message" in u:
                    ...
                    if state == "awaiting_p2p_rate":
                        clear_state(chat_id)
                        send(chat_id, "✅ Rate received. Thank you!", BACK_MAIN)
                        continue
