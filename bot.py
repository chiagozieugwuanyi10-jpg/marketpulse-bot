...
- FIXED: Removed duplicate handle_admin_command
- FIXED: Removed admin code requirement (now only checks ADMIN_IDS)
"""
...
# ── SINGLE ADMIN COMMAND HANDLER (FIXED - NO CODE CHECK) ─────────────────────
def handle_admin_command(chat_id, text):
    """Admin commands - only checks if user is in ADMIN_IDS."""
    ...
    if cmd == "/stats":
        ...
        if len(parts) < 2:
            send(chat_id, "Usage: /grantpro CHATID")
            return
        try:
            target = int(parts[1])
            grant_pro(target, chat_id)
            send(chat_id, "✅ Pro granted to %d" % target)
            send(target, "⭐ <b>Welcome to Market Pulse Pro!</b>")
        except Exception as e:
            send(chat_id, "Error: %s" % e)
    elif cmd == "/revokepro":
        if len(parts) < 2:
            send(chat_id, "Usage: /revokepro CHATID")
            return
        try:
            target = int(parts[1])
            revoke_pro(target)
        ...
        if len(parts) < 5:
            send(chat_id, "Usage: /rate CRYPTO FIAT BUY SELL")
            return
        try:
            ...
            submit_community_rate(chat_id, crypto, fiat, buy_rate, sell_rate, "Admin", is_admin=True)
            fiat_sym = P2P_FIATS.get(fiat, (fiat, fiat))[1]
            send(chat_id, "✅ Rate submitted as admin\n%s/%s — Buy %s%s  Sell %s%s" % (
                crypto, fiat, fiat_sym, "{:,.0f}".format(buy_rate), fiat_sym, "{:,.0f}".format(sell_rate)))
        except Exception as e:
            send(chat_id, "Error: %s" % e)
    elif cmd == "/publish":
        ...
        try:
            with get_db_cursor() as c:
                c.execute("SELECT data_json FROM weekly_data WHERE published=0 ORDER BY id DESC LIMIT 1")
                row = c.fetchone()
        except Exception:
            row = None
        if not row:
        ...
        try:
            with get_db_cursor() as c:
                c.execute("SELECT id, crypto, fiat, buy_rate, sell_rate, exchange, confirmations, timestamp FROM community_p2p WHERE status='pending' ORDER BY timestamp DESC LIMIT 10")
                rows = c.fetchall()
        except Exception:
            rows = []
        if not rows:
        ...
        for rid, cr, fi, buy, sell, ex, conf, ts in rows:
            ...
            lines.append("ID:%d  %s/%s  Buy:%s%s  Sell:%s%s  %d/%d conf  %s" % (
                rid, cr, fi, fiat_sym, "{:,.0f}".format(buy), fiat_sym, "{:,.0f}".format(sell), conf, P2P_CONSENSUS_NEED))
        send(chat_id, "\n".join(lines))
    elif cmd == "/approve":
        if len(parts) < 2:
            send(chat_id, "Usage: /approve RATE_ID")
            return
        try:
            rate_id = int(parts[1])
            with get_db_cursor() as c:
        ...
        if len(parts) < 2:
            send(chat_id, "Usage: /reject RATE_ID")
            return
        try:
            rate_id = int(parts[1])
            with get_db_cursor() as c:
            ...
            send(chat_id, "✅ Rate %d rejected." % rate_id)
        except Exception as e:
            send(chat_id, "Error: %s" % e)
    elif cmd == "/blocked":
        try:
            with get_db_cursor() as c:
                ...
                c.execute("SELECT chat, strikes_today, blocked_until FROM rate_submissions WHERE blocked_until > ? ORDER BY blocked_until DESC", (now_str,))
                rows = c.fetchall()
        except Exception:
            rows = []
        if not rows:
        ...
        for bchat, strikes, blocked_until in rows:
            lines.append("Chat: %s  Strikes: %d  Until: %s" % (bchat, strikes or 0, blocked_until[:16] if blocked_until else "Never"))
        send(chat_id, "\n".join(lines))
    elif cmd == "/unblock":
        if len(parts) < 2:
            send(chat_id, "Usage: /unblock CHATID")
            return
        try:
            target = int(parts[1])
            with get_db_cursor() as c:
                c.execute("UPDATE rate_submissions SET blocked_until=NULL, strikes_today=0 WHERE chat=?", (str(target),))
        ...
        try:
            with get_db_cursor() as c:
                c.execute("SELECT id, chat, reason, status, created_at FROM appeals WHERE status='pending' ORDER BY created_at ASC LIMIT 20")
                rows = c.fetchall()
        except Exception:
            rows = []
        if not rows:
        ...
        for aid, achat, reason, status, created in rows:
            lines.append("ID:%d  Chat:%s  Reason:%s  Created:%s" % (aid, achat, reason[:50], created[:16]))
        send(chat_id, "\n".join(lines))
    elif cmd == "/appeal_approve":
        if len(parts) < 2:
            send(chat_id, "Usage: /appeal_approve APPEAL_ID")
            return
        try:
            ...
            with get_db_cursor() as c:
                ...
                row = c.fetchone()
                if row:
                    c.execute("UPDATE appeals SET status='approved', resolved_at=? WHERE id=?", (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), appeal_id))
                    c.execute("UPDATE rate_submissions SET blocked_until=NULL, strikes_today=0 WHERE chat=?", (row[0],))
                    send(int(row[0]), "✅ Your appeal has been approved! You are now unblocked.")
            send(chat_id, "✅ Appeal approved and user unblocked.")
        except Exception as e:
            send(chat_id, "Error: %s" % e)
    elif cmd == "/appeal_reject":
        if len(parts) < 2:
            send(chat_id, "Usage: /appeal_reject APPEAL_ID")
            return
        try:
            appeal_id = int(parts[1])
            with get_db_cursor() as c:
                ...
                row = c.fetchone()
                if row:
                    c.execute("UPDATE appeals SET status='rejected', resolved_at=? WHERE id=?", (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), appeal_id))
                    send(int(row[0]), "❌ Your appeal has been rejected.")
            send(chat_id, "✅ Appeal rejected.")
        except Exception as e:
            send(chat_id, "Error: %s" % e)
    else:
        send(chat_id, f"✅ Admin command {cmd} received")
