"""
Market Pulse Bot — v23 "Admin Cleanup + Trade Analytics"
============================================
...
def _get_key_alert_cooldown(coin, level):
    """Return True if this coin+level is still in cooldown. DB-backed."""
    db = None
    try:
        db = get_db()
        c = db.cursor()
        ...
        c.execute(
            "SELECT id FROM admin_settings WHERE key=%s AND value=%s AND updated_at >= %s",
            (f"key_alert_{coin}", str(round(level, 2)), since)
        )
...
def _set_key_alert_cooldown(coin, level):
    """Record that we just alerted for this coin+level."""
    db = None
    try:
        db = get_db()
        c = db.cursor()
        ...
        c.execute(
            "INSERT INTO admin_settings (key, value, updated_at) VALUES (%s,%s,%s) "
            "ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at",
            (f"key_alert_{coin}", str(round(level, 2)), now)
        )
        db.commit()
    except Exception as e:
...
def check_key_market_alerts():
    ...
    try:
        for coin in KEY_ALERT_COINS:
            ...
            if not level:
                continue
            # DB-backed cooldown — survives bot restarts on Railway
            if _get_key_alert_cooldown(coin, level):
                logger.debug(f"[KEY ALERT] {coin} @ {format_price(level)} in cooldown, skipping")
                continue
        ...
        for proximity, coin, price, ch, level in triggered:
            if sent >= MAX_ALERTS_PER_CYCLE:
                break
            # Record cooldown BEFORE sending to avoid double-post on error
            _set_key_alert_cooldown(coin, level)
